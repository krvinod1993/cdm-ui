...
function MyCars() {
  ...
  const statusStyles = {
    active: 'bg-emerald-100 text-emerald-700 ring-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:ring-emerald-500/20',
    inactive: 'bg-gray-100 text-gray-600 ring-gray-200 dark:bg-gray-600/15 dark:text-gray-400 dark:ring-gray-600/30',
    sold: 'bg-rose-100 text-rose-700 ring-rose-200 dark:bg-rose-500/10 dark:text-rose-400 dark:ring-rose-500/20',
    draft: 'bg-amber-100 text-amber-700 ring-amber-200 dark:bg-amber-500/10 dark:text-amber-400 dark:ring-amber-500/20',
  };

  const getStatusBadge = (vehicleStatus) => {
    const cls = statusStyles[vehicleStatus] || statusStyles.inactive;
    return (
      <span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider ring-1 ${cls}`}>
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api(`/vehicles/${deleteTarget.id}`, { method: 'DELETE' });
      ...
      {!loading && !error && vehicles.length === 0 && (
        <div className="flex flex-col items-center gap-4 rounded-2xl border border-dashed border-gray-300 bg-white py-20 dark:border-slate-800 dark:bg-slate-950/40">
          ...
          <div className="text-center">
            <p className="font-medium text-gray-700 dark:text-slate-300">No vehicles listed yet</p>
            <p className="mt-1 text-sm text-gray-400 dark:text-slate-500">Add your first vehicle to get started.</p>
          </div>
          <Link
            to="/dealer/vehicles/add"
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50 dark:border-slate-800 dark:bg-slate-900/80">
                  <th className="px-5 py-4 text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-slate-500">Vehicle</th>
                  ...
                  <th className="px-5 py-4 text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-slate-500">Status</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-800/60">
                {vehicles.map((vehicle) => {
                  const imgSrc = vehicle.image_url
                    ...
                    : null;
                  return (
                    <tr key={vehicle.id} className="bg-white transition-colors hover:bg-gray-50 dark:bg-slate-900/40 dark:hover:bg-slate-800/50">
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="mt-0.5 text-xs text-gray-400 sm:hidden dark:text-slate-500">{vehicle.brand}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <select
                            ...
                            className="rounded-md border border-gray-300 bg-white px-2 py-1 text-[11px] font-medium text-gray-700 outline-none transition focus:border-sky-500 focus:ring-1 focus:ring-sky-500 disabled:opacity-60 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"
                          >
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            ...
                            className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-xs font-semibold text-gray-600 transition hover:border-gray-300 hover:bg-gray-100 dark:border-gray-600 dark:bg-gray-700/40 dark:text-gray-300 dark:hover:border-gray-500 dark:hover:bg-gray-700"
                          >
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md animate-modal-in rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl sm:p-8 dark:border-slate-800 dark:bg-slate-900">
            ...
            <h3 className="text-center text-lg font-semibold text-gray-900 dark:text-slate-50">Delete Vehicle</h3>
